...
export async function getOrderForPayment(orderNumber) {
  const normalizedOrder = normalizeOrderNumber(orderNumber);
  if (!normalizedOrder) throw new Error("Order number is required.");
  if (!hasSupabasePaymentConfig()) throw new Error("Supabase is not configured.");
  const record = await findOrderRecord(normalizedOrder);
  if (!record?.id) throw new Error(`Order ${normalizedOrder} was not found.`);
  if (!["pending_payment", "payment_failed"].includes(String(record.status || ""))) {
    throw new Error("This order is not available for a new payment.");
  }
  const amount = moneyToPayPalValue(record.amount);
  if (!amount) throw new Error("The stored order amount is invalid.");
  return { orderNumber: normalizedOrder, amount, record };
}

export async function markStripeOrderPaid({ orderNumber, session, eventId }) {
  ...
  const patch = {
    ...
    stripe_event_id: clean(eventId, 180),
    paid_at: paidAt,
  };
...
export async function markStripeOrderFailed({ orderNumber, paymentIntent, eventId }) {
  return transitionStripeOrder({ orderNumber, allowed: ["pending_payment"], status: "payment_failed", paymentIntentId: paymentIntent?.id, eventId, note: "Stripe payment failed" });
}

export async function markStripeOrderRefunded({ orderNumber, charge, paymentIntent, eventId }) {
  return transitionStripeOrder({ orderNumber, allowed: ["paid"], status: "refunded", paymentIntentId: paymentIntent?.id || charge?.payment_intent, chargeId: charge?.id, eventId, note: "Stripe charge refunded", refunded: true });
}

async function transitionStripeOrder({ orderNumber, allowed, status, paymentIntentId, eventId, note, refunded = false, chargeId = "" }) {
  const normalizedOrder = normalizeOrderNumber(orderNumber);
  if (!normalizedOrder || !hasSupabasePaymentConfig()) return { saved: false, warning: "Order or Supabase configuration is missing." };
  const record = await findOrderRecord(normalizedOrder);
  if (!record?.id) return { saved: false, warning: "Order was not found." };
  if (record.stripe_event_id === eventId || !allowed.includes(record.status)) return { saved: true, duplicate: true };
  const now = new Date().toISOString();
  const response = await supabaseFetch(`${ORDER_TABLE}?id=eq.${encodeURIComponent(record.id)}&status=in.(${allowed.join(",")})`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({ status, payment_status: status, payment_provider: "stripe", stripe_payment_intent_id: clean(paymentIntentId, 180), stripe_charge_id: clean(chargeId, 180), stripe_event_id: clean(eventId, 180), ...(refunded ? { refunded_at: now } : {}), updated_at: now }),
  });
  return { saved: response.ok, warning: response.ok ? "" : await safeResponseMessage(response) };
}
...
async function findOrderRecord(orderNumber) {
  const query = new URLSearchParams({
    select: "id,title,description,status,amount,customer_name,customer_email,type,stripe_event_id",
    type: "eq.order",
