...
export function sanitizeCheckoutInput(input) {
  return {
    internalOrderId: clean(input.internalOrderId, 80),
...
export async function verifyPayPalWebhook(req, event) {
  const webhookId = clean(process.env.PAYPAL_WEBHOOK_ID, 120);
...
export function publicBaseUrl(req = {}, env = process.env) {
  const requestHost = clean(req.headers?.host, 260);
  // Vercel Dev can provide production-like environment variables while its request host
  // is local. The request host must win so PayPal never redirects to HTTPS localhost.
  if (isLocalHost(requestHost)) return `http://${requestHost}`;

  const envUrl = env.NEXT_PUBLIC_SITE_URL || env.VERCEL_PROJECT_PRODUCTION_URL || env.VERCEL_URL;
  if (envUrl) {
    const normalized = normalizePublicUrl(envUrl);
    if (isLocalHost(hostFromUrl(normalized))) return normalized.replace(/^https:/, "http:");
    return normalized;
  }

  if (requestHost) return `https://${requestHost}`;
  return "http://localhost:4178";
}
